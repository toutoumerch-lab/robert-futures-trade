...
const BlogDetail = () => {
  ...
  const handleReaction = async (type) => {
    ...
    setRxLoad(type);
    try {
      const res = await axios.post(`http://localhost:5000/api/posts/${id}/reactions`, { type });
  ...
  const handleComment = async (e) => {
    ...
    setSub(true);
    try {
      const res = await axios.post(`http://localhost:5000/api/posts/${id}/comments`, { content: newComment });
