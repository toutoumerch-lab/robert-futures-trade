...
const CourseLearn = () => {
    ...
    const loadCourse = async () => {
      try {
        ...
        const enrollCheck = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/enrollments/check/${id}`,
  ...
  const toggleLessonStatus = async () => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/enrollments/progress`, {
                ...
                <div className="learn-lesson-resources">
                  ...
                  <div className="learn-lesson-resources__list">
                    {activeLesson.pdf_url && (
                      <a href={`${import.meta.env.VITE_API_URL}${activeLesson.pdf_url}`} target="_blank" rel="noreferrer" className="learn-resource-link learn-resource-link--pdf">
                        <FileDown size={16} /> Download PDF
                      </a>
                    )}
                    {activeLesson.zip_url && (
                      <a href={`${import.meta.env.VITE_API_URL}${activeLesson.zip_url}`} target="_blank" rel="noreferrer" className="learn-resource-link learn-resource-link--zip">
