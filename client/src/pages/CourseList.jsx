...
import { BookOpen, SearchX, Bookmark, Clock, ArrowRight, Search, X, Loader } from 'lucide-react';
...
const CourseList = () => {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      axios.get(`http://localhost:5000/api/courses/search?q=${encodeURIComponent(searchQuery)}`)
    ...
    <div>
      ...
      <section className="page-hero" style={{ padding: '6rem 0', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container text-center" style={{ position: 'relative', zIndex: 1 }}>
          ...
          {/* Search Bar — inside hero for visual impact */}
          <div ref={searchContainerRef} style={{ position: 'relative', maxWidth: '640px', margin: '0 auto', zIndex: 50 }}>
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              ...
              {searchQuery && (
                <button
                  ...
                  style={{ position: 'absolute', right: '18px', background: 'transparent', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', display: 'flex', alignItems: 'center', padding: '4px', borderRadius: '50%', transition: 'color 0.2s' }}
                >
                  <X size={18} />
                </button>
              )}
            ...
            {showDropdown && hasQueried && (
              <div style={{
                ...
                boxShadow: '0 20px 60px rgba(0,0,0,0.5)', overflow: 'hidden', textAlign: 'left'
              }}>
                {isSearching ? (
                  ...
                  <>
                    <div style={{ padding: '0.75rem 1.5rem 0.5rem', fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--text-secondary)', borderBottom: '1px solid var(--border)' }}>
                      Top Results
                    </div>
                    {searchResults.map(res => (
                      ...
                      >
                        <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: res.image_url ? `url(http://localhost:5000${res.image_url}) center/cover` : 'var(--bg-secondary)', flexShrink: 0, border: '1px solid var(--border)' }} />
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '2px' }}>
                            {res.category || 'General'} · {res.level || 'Beginner'} · {res.is_free ? 'FREE' : `$${res.price}`}
                          </div>
                        </div>
