  ...
  return (
    <div>
      ...
      <section className="page-hero" style={{ padding: '6rem 0', position: 'relative', overflow: 'hidden' }}>
        <div className="page-hero-bg" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'linear-gradient(180deg, rgba(168, 85, 247, 0.05) 0%, transparent 100%)', zIndex: 0 }} />
          ...
          <div style={{ display: 'flex', gap: '0.75rem', overflowX: 'auto', paddingBottom: '2rem', marginBottom: '2rem', borderBottom: '1px solid var(--border)' }} className="scrollbar-hide">
             <button 
               ...
               style={{ 
                 ...
                 fontWeight: 800, 
                 whiteSpace: 'nowrap',
                 background: selectedCategory === 'All' ? 'var(--accent-purple)' : 'var(--bg-secondary)', 
                 color: selectedCategory === 'All' ? '#fff' : 'var(--text-secondary)',
                 border: selectedCategory === 'All' ? 'none' : '1px solid var(--border)',
                 transition: 'all 0.2s',
                 boxShadow: selectedCategory === 'All' ? '0 4px 15px rgba(168, 85, 247, 0.3)' : 'none'
               }}
             >
               All Masterclasses
             </button>
             {categories.map(cat => (
               <button 
                 ...
                 style={{ 
                   ...
                   fontWeight: 800, 
                   whiteSpace: 'nowrap',
                   background: selectedCategory === cat.name ? 'var(--accent-purple)' : 'var(--bg-secondary)', 
                   color: selectedCategory === cat.name ? '#fff' : 'var(--text-secondary)',
                   border: selectedCategory === cat.name ? 'none' : '1px solid var(--border)',
                   transition: 'all 0.2s',
                   boxShadow: selectedCategory === cat.name ? '0 4px 15px rgba(168, 85, 247, 0.3)' : 'none'
                 }}
               >
                 {cat.name}
               </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredCourses.map(course => (
                 <div 
                   ...
                   className="hover:-translate-y-2 hover:shadow-[0_20px_40px_-10px_rgba(236,72,153,0.15)] hover:border-[var(--accent-purple)]"
                 >
                   ...
                   <div style={{ padding: '2rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                     <div style={{ display: 'flex', gap: '8px', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
                       <span style={{ fontSize: '0.75rem', background: 'var(--bg-primary)', padding: '6px 14px', borderRadius: '99px', color: 'var(--accent-purple)', fontWeight: 800, border: '1px solid var(--border)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{course.level || 'Beginner'}</span>
                       <span style={{ fontSize: '0.75rem', background: 'var(--bg-primary)', padding: '6px 14px', borderRadius: '99px', color: 'var(--text-secondary)', fontWeight: 700, border: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '4px' }}>⏱ {course.duration || 'N/A'}</span>
                     </div>
                     ...
                     <div style={{ marginTop: 'auto' }}>
                       <Button style={{ width: '100%', padding: '1rem', borderRadius: '16px', fontSize: '1rem', fontWeight: 800, background: 'linear-gradient(135deg, var(--accent-pink), var(--accent-purple))', boxShadow: '0 8px 20px rgba(168, 85, 247, 0.3)' }}>
                         View Course →
                       </Button>
                     </div>
                   </div>
