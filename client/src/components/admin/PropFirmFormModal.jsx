...
// ─── Size Card ────────────────────────────────────────────────────────────────
const SizeCard = ({ size, onUpdate, onRemove, onCopyAll, showEval }) => {
  const { savings, afterDiscount } = calcPricing(size);
  const allIn = afterDiscount > 0 ? +(afterDiscount + (parseFloat(size.activation_fee) || 0)).toFixed(2) : 0;
  const u = (patch) => onUpdate(patch);
  const cp = (field) => onCopyAll(field, size[field]);

  return (
    <div style={{ border: '1px solid var(--border-color)', borderRadius: '10px', overflow: 'hidden', marginBottom: '0.6rem' }}>
      {/* Size header */}
      <div style={{ display: 'flex', alignItems: 'center', padding: '0.55rem 0.85rem', background: 'var(--bg-secondary)', cursor: 'pointer', gap: '0.6rem', userSelect: 'none' }}
        onClick={() => u({ expanded: !size.expanded })}>
        {size.expanded ? <ChevronDown size={14} style={{ color: 'var(--text-secondary)', flexShrink: 0 }} /> : <ChevronRight size={14} style={{ color: 'var(--text-secondary)', flexShrink: 0 }} />}
        <span style={{ fontWeight: 800, fontSize: '0.9rem', color: 'var(--text-primary)' }}>${size.label || '—'}</span>
        {afterDiscount > 0 && <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>${afterDiscount.toFixed(2)}</span>}
        <div style={{ flex: 1 }} />
        <button type="button" onClick={e => { e.stopPropagation(); onRemove(); }}
          style={{ color: '#ef4444', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: '6px', padding: '3px 7px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
          <Trash2 size={12} />
        </button>
      </div>
        ...
        <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          {showEval && (
            <div>
              ...
              {(size.eval_max_mini || size.eval_max_micro) && (
                <p style={{ marginTop: '0.5rem', fontSize: '0.74rem', color: 'var(--text-secondary)', fontStyle: 'italic' }}>
                  Eval contract limits: <strong>{size.eval_max_mini || '—'} Mini</strong> or <strong>{size.eval_max_micro || '—'} Micro</strong>
                </p>
              )}
            </div>
          )}
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.65rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                {size.funded_profit_split && !isNaN(parseFloat(size.funded_profit_split)) && (
                  <p style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', margin: 0 }}>
                    {size.funded_profit_split}/{100 - parseFloat(size.funded_profit_split)} — trader {size.funded_profit_split}% · firm {100 - parseFloat(size.funded_profit_split)}%
                  </p>
                )}
              </div>
            </div>
            {(size.funded_max_mini || size.funded_max_micro) && (
              <p style={{ marginTop: '0.5rem', fontSize: '0.74rem', color: 'var(--text-secondary)', fontStyle: 'italic' }}>
                Funded contract limits: <strong>{size.funded_max_mini || '—'} Mini</strong> or <strong>{size.funded_max_micro || '—'} Micro</strong>
              </p>
            )}
          </div>
...
const PlanCard = ({ plan, onUpdate, onRemove, onCopy }) => {
  ...
  const removeSize = (sIdx) => {
    ...
    onUpdate({ sizes: plan.sizes.filter((_, i) => i !== sIdx) });
  };

  const addSize = (label = '') => {
    onUpdate({ sizes: [...plan.sizes, makeSize(label)] });
  ...
  return (
    <div style={{ border: '1px solid var(--border-color)', borderRadius: '12px', marginBottom: '0.85rem', overflow: 'visible' }}>
      {/* Plan header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.65rem 0.85rem', background: 'var(--bg-secondary)', borderRadius: plan.expanded ? '12px 12px 0 0' : '12px', flexWrap: 'wrap' }}>
        <button type="button" onClick={() => onUpdate({ expanded: !plan.expanded })}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-secondary)', display: 'flex', padding: 0, flexShrink: 0 }}>
          {plan.expanded ? <ChevronDown size={15} /> : <ChevronRight size={15} />}
        </button>

        <input className="input" value={plan.name} onChange={e => onUpdate({ name: e.target.value })}
          placeholder="Plan name..." onClick={e => e.stopPropagation()}
          style={{ width: '160px', fontWeight: 700, fontSize: '0.88rem', padding: '5px 9px' }} />
        ...
        <select value={plan.type} onChange={e => onUpdate({ type: e.target.value })}
          style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: '6px', padding: '5px 8px', fontSize: '0.8rem', color: 'var(--text-primary)', outline: 'none' }}>
          <option value="evaluation_funded">Evaluation → Funded</option>
          <option value="straight_funded">Straight to Funded</option>
          <option value="one_step">1-Step Evaluation</option>
          <option value="two_step">2-Step Evaluation</option>
        </select>

        <span style={{ background: 'rgba(255,255,255,0.06)', padding: '2px 10px', borderRadius: '99px', fontSize: '0.7rem', fontWeight: 800, color: 'var(--text-secondary)', flexShrink: 0 }}>
          {plan.sizes.length} size{plan.sizes.length !== 1 ? 's' : ''}
        </span>
        ...
        <div style={{ padding: '0.85rem', borderTop: '1px solid var(--border-color)' }}>
          {plan.sizes.map((size, sIdx) => (
            <SizeCard
              key={size._key}
              size={size}
              onUpdate={patch => updateSize(sIdx, patch)}
              onRemove={() => removeSize(sIdx)}
              onCopyAll={(field, value) => copyAll(sIdx, field, value)}
              showEval={showEval}
            />
          ))}
        </div>
    ...
    try {
      for (const plan of plans) {
        for (const size of plan.sizes) {
          ...
          fd.append('price',              afterDiscount || size.price_no_discount);
          ...
          fd.append('fifty_k_initial_cost', afterDiscount || size.price_no_discount);
          fd.append('profit_target',      size.eval_profit_target);
          fd.append('dll',                size.eval_dll);
          fd.append('drawdown_limit',     size.eval_max_loss ? `${size.eval_drawdown_type} $${size.eval_max_loss}` : size.eval_drawdown_type);
          ...
          fd.append('notes',              JSON.stringify({
            plan_type: plan.type, plan_variant: plan.variant,
            eval_consistency: size.eval_consistency_percent,
            eval_max_inactive: size.eval_max_inactive_days,
            funded_dll: size.funded_dll, funded_max_loss: size.funded_max_loss,
          ...
          if (editing) {
            await axios.put(`${API}/api/prop-firms/${editing.id}`, fd);
          } else {
            await axios.post(`${API}/api/prop-firms`, fd);
          }
        }
      }
      onSaved();
